...
uint8_t nuevo_estado = 0;

volatile uint16_t ADCValor[8];
...
int main(void)
{
  ...
  MX_TIM6_Init();
  /* USER CODE BEGIN 2 */
  HAL_ADC_Start_DMA(&hadc1, (uint32_t *)ADCValor, 8);
...
//	  }
	  ...
	  if (contador == 1)
	  {
	      ...
	      uint8_t nuevo_estado = 0; // Se limpia cada vez antes de recalcular

	      for (int i = 0; i < 8; i++)
	      {
	          if (ADCValor[i] >= 1000) // Cuando el sensor detecta >= 1000 activa el sensor correspondiente
	          {
	              nuevo_estado |= (1 << i);
	          }
	      }

	      sensor_state = nuevo_estado;
	  }
  }
  /* USER CODE END 3 */
}
